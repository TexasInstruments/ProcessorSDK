        ...
        vx_df_image dst_format)
{
    ...
    if(0 != ct_image_read_rect_S32(src, &value, x, y, x, y, border))
    {
        return -1;
    }
...
#define PARAMETERS_MULTI \
    CT_GENERATE_PARAMETERS("random", ADD_SIZE_SMALL_SET, ADD_SET_TARGET_PARAMETERS_MULTI_INST, ADD_KERNEL_SIZE, ADD_VX_BORDERS_REQUIRE_UNDEFINED_ONLY, ARG, halfScaleGaussian_generate_random, NULL), \
    CT_GENERATE_PARAMETERS("lena", ADD_SIZE_NONE, ADD_SET_TARGET_PARAMETERS_MULTI_INST, ADD_KERNEL_SIZE, ADD_VX_BORDERS_REQUIRE_UNDEFINED_ONLY, ARG, halfScaleGaussian_read_image, "lena.bmp"), \

TEST_WITH_ARG(tivxHwaVpacMscHalfScaleGaussian, testGraphProcessing_multi, Arg_multi,
    PARAMETERS_MULTI
)
{
    vx_context context = context_->vx_context_;
    ...
    ASSERT(vx_true_e == tivxIsTargetEnabled(arg_->target_string));
    {
        ...
        for (i = 0; i < TEST_NUM_NODE_INSTANCE; i++)
        {
                 ...
                 ((i==1) && (NULL != arg_->target_string_2)) )
            {
                ...
                ASSERT_VX_OBJECT(node[i] = vxHalfScaleGaussianNode(graph, src_image[i], dst_image[i], arg_->kernel_size), VX_TYPE_NODE);
