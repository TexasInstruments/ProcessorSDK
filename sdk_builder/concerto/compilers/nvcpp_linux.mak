...
CC = nvc++ -c -fast -acc -gpu=ccall -tp=px -Minfo=accel -noswitcherror
...
LD = nvc++ -fast -acc -gpu=ccall -tp=px -noswitcherror
else
...
$(_MODULE)_LINK_DSO   := $(LD) $($(_MODULE)_CPLDFLAGS) -shared -Wl,--allow-multiple-definition -Wl,$(EXPORT_FLAG) -Wl,-soname,$(notdir $($(_MODULE)_BIN)).$($(_MODULE)_VERSION) $($(_MODULE)_OBJS) -Wl,--whole-archive $($(_MODULE)_LIBRARIES) -lm -Wl,--no-whole-archive -o $($(_MODULE)_BIN).$($(_MODULE)_VERSION) -Wl,-Map=$($(_MODULE)_MAP)
$(_MODULE)_LINK_EXE   := $(LD) $(EXPORTER) -Wl,--cref $($(_MODULE)_CPLDFLAGS) $($(_MODULE)_OBJS) -Wl,--unresolved-symbols=ignore-in-shared-libs $($(_MODULE)_LIBRARIES) -o $($(_MODULE)_BIN) -Wl,-Map=$($(_MODULE)_MAP)
endif
