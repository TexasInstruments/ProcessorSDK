...
@REM ******************************************************************************
@REM * FILE NAME: saProjectCreate.bat
@REM *
@REM * DESCRIPTION: 
...
@REM *
@REM * Syntax:
@REM *  saProjectCreate.bat [deviceName] [endian] [module] [pdkDir] 
@REM *
@REM * Description:     (first option is default)
@REM *  deviceName  -   /k2l /k2e /k2k /k2h
@REM *  endian      -   little / big
@REM *  module      -   sa
@REM *  pdkDir      -   THIS FILE LOCATION / "C:\ti\pdk_keystone2_3_01_00_00\packages"
@REM *
@REM * Example:
@REM *  a) saProjectCreate.bat
@REM *              - Creates sa module projects for k2k device for little endian
@REM *  b) saProjectCreate.bat k2l
@REM *              - Creates sa module projects for k2l device for little endian
@REM *  c) saProjectCreate.bat k2k big
@REM *              - Creates sa module projects for k2k device for big endian
@REM *
...
REM Valid values are 'k2k', 'k2h' or 'k2l'. Defaults to 'k2l'.
set tempVar1=%1
...
REM defined above.
set CCS_INSTALL_PATH="C:\ti\ccs5_5\ccsv5"

REM Workspace where the PDK projects will be created.
set MY_WORKSPACE=%PDK_SHORT_NAME%\MySaProjects
...
REM Version of XDC
set XDC_VERSION=3.25.02.70

REM Version of BIOS
set BIOS_VERSION=6.33.06.50

REM Version of the IPC
set IPC_VERSION=3.00.00.08
...
REM Search for all the test Project Files in SA.
...
echo *****************************************************************************
echo Detecting Example Projects in SA LLD package and importing them in the workspace %MY_WORKSPACE%
...
REM Execute the command to create the project using the parameters specified above.
%CCS_INSTALL_PATH%\%AUTO_CREATE_COMMAND% -data %MY_WORKSPACE% -application com.ti.ccstudio.apps.projectCreate -ccs.name %%~nI -ccs.outputFormat %OUTPUT_FORMAT% -ccs.device com.ti.ccstudio.deviceModel.C6000.GenericC64xPlusDevice -ccs.endianness %ENDIAN% -ccs.kind executable -ccs.cgtVersion %CGT_VERSION% -rtsc.xdcVersion %XDC_VERSION% -rtsc.enableDspBios -rtsc.biosVersion %BIOS_VERSION% -rtsc.buildProfile "debug" -rtsc.products "com.ti.rtsc.IPC:%IPC_VERSION%;com.ti.rtsc.SYSBIOS:%BIOS_VERSION%;com.ti.rtsc.XDAIS:%XDAIS_VERSION%;ti.pdk:%PDK_VERSION%;ti.drv.sa:%SA_VERSION%" -rtsc.platform "%RTSC_PLATFORM_NAME%" -rtsc.target %RTSC_TARGET% -ccs.rts libc.a -ccs.args %%~nI%%~xI %SIMULATOR_SUPPORT_DEFINE% -ccs.setCompilerOptions
