...
if [ "$($git_cmd config --get remote.origin.url | sed -E 's#.*/##')" = "device_manager" ]; then
	...
	dm_ver="$("$git_cmd" describe --match "REL.MCUSDK.*" --abbrev=8 --dirty | sed -e 's/-g........//g' -e 's/-dirty$/+/g' -e 's/^REL.MCUSDK/MSDK/g')"
